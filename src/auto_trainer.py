...
from torch.amp import GradScaler  # Updated import

# Check CUDA availability
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is not available. Please ensure you have a CUDA-capable GPU and the correct PyTorch version installed.")
...
try:
    ...
    print("Installing required transformers packages...")
    import subprocess
    subprocess.check_call(["pip", "install", "--upgrade", "torch==2.1.0", "torchvision==0.16.0", "transformers==4.35.0", "--index-url", "https://download.pytorch.org/whl/cu118"])
    from transformers import (
        AutoModelForCausalLM,
        AutoTokenizer,
        TrainingArguments,
...
# Configure paths for Google Drive integration
GDRIVE_MOUNT = Path("C:/mak_gdrive")  # Windows path for Google Drive mount
SAVE_DIR = GDRIVE_MOUNT / "logs"
CHROMA_DB_PATH = GDRIVE_MOUNT / "chroma_db"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(str(SAVE_DIR / 'auto_trainer.log')),
...
class AutoTrainer:
    def __init__(self):
        self.model_dir = GDRIVE_MOUNT / "models"  # Save models to Google Drive
        ...
        self.optimizer = None
        self.scaler = GradScaler('cuda')  # Force CUDA for GradScaler
        ...
        file_handler = logging.FileHandler(SAVE_DIR / 'auto_trainer.log')
    ...
    def load_trained_topics_log(self):
        ...
        try:
            trained_log_path = SAVE_DIR / "trained_topics_log.json"
